...
class Category(models.Model):
    ...
    slug = models.SlugField(
        "Slug",
        max_length=50, unique=True, help_text='根据name生成的，用于生成页面url，必须唯一'
    )
    description = models.TextField("描述")
    is_active = models.BooleanField("是否激活", default=True)
    meta_keywords = models.CharField(
        "Meta 关键字", max_length=255, help_text='meta 关键字，有利于seo，用逗号分隔'
    )
    meta_description = models.CharField(
        "Meta 描述", max_length=255, help_text='meta 描述'
    )
...
class Product(models.Model):
    ...
    slug = models.SlugField(
        "Slug",
        max_length=255, unique=True, help_text='根据name生成的，用于生成页面url，必须唯一'
    )
